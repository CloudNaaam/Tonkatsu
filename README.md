...
| `데이터셋 구축`<br>`Feature Engineering`<br>`SVM 학습`<br>`디스코드 봇 구현 및 배포`                                                                             | `데이터셋 구축`<br>`Feature Engineering`<br>`XGB 학습`<br>`기술적 피드백`                                                                                | `데이터셋 구축`<br>`Feature Engineering`<br>`Random Forest 학습`<br>`데이터셋 분류&연구`                                                                 | `데이터셋 구축`<br>`Feature Engineering`<br>`LGBM 학습`                                                                                                    | `데이터셋 구축`<br>`Decision Tree 학습`<br>`학습 모델 분류`<br>`데이터셋 관리`                                                                           |
...
# 느낀 점
>
> 인생 첫 프로젝트였는데.. PL이라는 중책을 맡게 되었습니다.<br>
> 협업 툴이라는 게 뭔지도 모르고, 그냥 말 그대로 도화지에서 시작해서 그런가 참 힘들었습니다.<br>
> 그래도 나름 첫 프로젝트라고 애정이 가서 그런가 플젝 기간동안은 오롯이 프로젝트에만 집중했던 것 같습니다.<br>
> 성장하기 위한 첫 단계는 인정이라고 했던가요. 제 자신이 얼마나 부족한 지 알게 되니, 곧바로 부족함을 인정하게 되더군요.<br>
> 그리고 그만큼 노력했습니다. 1인분이라도 하려고, 아니, 그래도 PL인데 1인분보다는 더 하고 싶어서요.<br>
> 결국 꾸준한 멘토님의 피드백과 팀원들 간 노력으로 프로젝트는 성공적으로 마쳤습니다.<br>
> 우여곡절이 많았지만, 그만큼 얻는 것도 정말 많았던 프로젝트였습니다. 정말로요.<br>
> 항상 저희에게 딱 필요한 피드백을 주셨던 박수현 멘토님 감사드립니다.<br>
> 그리고 끝까지 프로젝트 함께한 팀원들 정말 고생 많았어요!<br>
>
